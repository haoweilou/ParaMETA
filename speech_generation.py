...
wave,_,_,_ = model.infer(ipa_index,style,x_lengths=src_lens,length_scale=1,noise_scale=0.5,noise_scale_w=0,ab_style=speech_embed)
...
wave,_,_,_ = model.infer(ipa_index,style,x_lengths=src_lens,length_scale=1,noise_scale=0.5,noise_scale_w=0,ab_style=text_embed)
save_audio(wave[0].cpu().detach(), 22050, f"text_ch","./generation/")

en = "how are you doing today? I hope you have a great day!"
ipa,style = mix_to_ipa(en)
ipa_index = torch.tensor([ipa_to_idx(ipa)]).to(device)  
style = torch.tensor([style]).to(device)
src_lens = torch.tensor([ipa_index.shape[-1]]).to(device)
wave,_,_,_ = model.infer(ipa_index,style,x_lengths=src_lens,length_scale=1,noise_scale=0.5,noise_scale_w=0,ab_style=speech_embed)
