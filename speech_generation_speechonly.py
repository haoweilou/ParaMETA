...
mel1 = utils.load_wav_to_mel("./generation/speech_only/reference1.wav").unsqueeze(0).to(device)
if mel1.shape[-1] % 4 != 0:  mel1 = mel1[:, :, :mel1.shape[-1] - (mel1.shape[-1] % 4)]
mel2 = utils.load_wav_to_mel("./generation/speech_only/reference2.wav").unsqueeze(0).to(device)
...
utils.save_audio(wave[0].cpu().detach(), 22050,"speech_ch_female", "./generation/speech_only/")
wave,_,_,_ = model.infer(ipa_index,tone,x_lengths=src_lens,length_scale=1,noise_scale=0.5,noise_scale_w=0,mel=mel2)
utils.save_audio(wave[0].cpu().detach(), 22050,"speech_ch_male", "./generation/speech_only/")
...
src_lens = torch.tensor([ipa_index.shape[-1]]).to(device)
wave,_,_,_ = model.infer(ipa_index,tone,x_lengths=src_lens,length_scale=1,noise_scale=0.5,noise_scale_w=0,mel=mel1)
utils.save_audio(wave[0].cpu().detach(), 22050,"speech_en_female", "./generation/speech_only/")
wave,_,_,_ = model.infer(ipa_index,tone,x_lengths=src_lens,length_scale=1,noise_scale=0.5,noise_scale_w=0,mel=mel2)
utils.save_audio(wave[0].cpu().detach(), 22050,"speech_en_male", "./generation/speech_only/")